...
class RPGInventory(commands.Cog):
    ...
    async def inv(self, ctx):
        ...
        embed.add_field(
            name="<:2851:1495250164116492469> Vật phẩm",
            value="\n".join(item_lines) or "_Trống_",
            inline=False,
        )
        if crate_lines:
        ...
        embed.add_field(
            ...
            value="\n".join(weapon_lines) or "_Trống_",
            inline=False,
        )
        if upgraded_lines:
            embed.add_field(
                name="<:Info:1496098636247863491> Weapon Nâng Cấp",
                value="\n".join(upgraded_lines),
                inline=False,
            )
...
class RPGShop(commands.Cog):
    ...
    async def shop_item(self, ctx):
        embed = discord.Embed(
            ...
            description="Vật phẩm kiếm được qua `dtn hunt`. Bán bằng `dtn sell item <id>`.",
            color=0x5865F2,
        )
        ...
        for rarity in rarity_order:
            ...
            embed.add_field(name=_rarity_tier(rarity), value="\n".join(lines), inline=False)

        embed.set_footer(text="Giá thực tế ngẫu nhiên trong range. Weapon trang bị tăng giá bán.")
        await ctx.send(embed=embed)
    ...
    async def shop_weapon(self, ctx):
        ...
        for s in shop["slots"]:
            ...
            embed.add_field(
                name=f"`[{s['slot']:02d}]` {current_emoji} {s['name']}  {rarity_e}",
                value=(

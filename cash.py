...
_eco_lock = _asyncio.Lock()

ECO_FILE  = "economy.json"
ECO_TMP   = "economy.json.tmp"
ECO_BAK   = "economy.json.bak"
...
def load_eco():
    ...
    with open(ECO_FILE, "r", encoding="utf-8") as f:
        try:
            ...
            # File hỏng → trả về dict rỗng, không crash bot
            return {}

def save_eco(data):
    # Ghi ra .tmp trước, rename sau → tránh corrupt nếu bị kill giữa chừng
    with open(ECO_TMP, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False)
    # Backup bản cũ trước khi replace
    if os.path.exists(ECO_FILE):
        os.replace(ECO_FILE, ECO_BAK)
    os.replace(ECO_TMP, ECO_FILE)

def get_balance(user_id):
    data = load_eco()
    return data.get(str(user_id), 0)

async def update_balance_safe(user_id, amount):
    """Luôn dùng hàm này thay cho update_balance để tránh race condition."""
...
# Giữ lại hàm sync để không break chỗ nào chưa migrate (nhưng không dùng trong Cog nữa)
...
class Cash(commands.Cog):
    ...
    async def spin(self, ctx, amount: str):
        ...
        if is_win:
            winnings = amount * 2
            await update_balance_safe(ctx.author.id, winnings)

...
class RPGRepair(commands.Cog):
    ...
    async def repair(self, ctx, target_uid: str = None):
        ...
        # Báo thiếu tiền ngay trên embed nhưng vẫn giữ nút để UX rõ ràng.
        if not can_afford:
            embed.color = 0xFF0000
            embed.add_field(
                name=f"{ERR} Không đủ tiền",
                value=(
                    f"Cần: **{total_cost:,}** {COIN_EMOJI}\n"
                    f"Có:   **{balance:,}** {COIN_EMOJI}\n"
                    f"Thiếu: **{total_cost - balance:,}** {COIN_EMOJI}"
                ),
                inline=False,
            )
            ...
            async def btn_confirm(self_v, interaction: discord.Interaction, _btn):
                ...
                if _bal < total_cost:
                    return await interaction.response.send_message(
                        f"{ERR} Không đủ tiền!\n"
                        f"Cần **{total_cost:,}** {COIN_EMOJI} │ "
                        f"Có **{_bal:,}** {COIN_EMOJI} │ "
                        f"Thiếu **{total_cost - _bal:,}** {COIN_EMOJI}",
                        ephemeral=True,
                    )
